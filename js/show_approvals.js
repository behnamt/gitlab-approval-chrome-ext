...
function handleMergeRequestApprovalInjection (mergeRequestId, mergeRequest, requestView, awardEmojis) {
  ...
  if (compactApprovals) {
    injectApprovalListCompact(requestView, mergeRequest.approvals_required - mergeRequest.approved_by.length - mergeRequest.approvers.length, mergeRequest.approvers, mergeRequest.approved_by, awardEmojis)
  } else {
 ...
 * @param {Element} requestView the HTML element reference to the Merge Request row.
 * @param {Integer} requiredApprovalsLeft the number of approvals left to allow for a merge.
 * @param {List<GitLab.User>} approvedUsers the users that have approved this Merge Request.
 */
function injectApprovalListCompact (requestView, requiredApprovalsLeft, approvers, approvedUsers, awardEmojis) {
  // Create the initial divs that wrap the approvals -- similar to the one found when viewing the MR
  let listContainer = `<div class="approved-by-users approvals-footer"><div class="approvers-prefix"><div class="approvers-list">`

  // Approved
  approvedUsers.forEach(entry => {
  ...
  approvers.forEach(entry => {
    var emoji = awardEmojis.find(awardEmoji => awardEmoji.userId === entry.user.id)
    listContainer += createNonApprovalDiv(entry.user, emoji, approvedUsers)
  })
...
function createApprovalDiv (user, emoji) {
  ...
  <div class='link-to-member-avatar'>
    <a class="author_link has-tooltip approver-avatar js-approver-list-member" data-container="body" href="/${user.username}" data-original-title="Approved by ${user.name}">
      <img width="30" class="avatar avatar-inline s20 js-lazy-loaded w30 green" alt="" src="${user.avatar_url}">
      <div class="avatar-award-emoji">${emoji ? emoji.name : ''}</div>
    </a>
  </div>`
}

function createNonApprovalDiv (user, emoji, allApproved) {
  if (user){
    allApproved = allApproved || [];
    var index = allApproved.findIndex(approved => approved.user.username === user.username);
    if (index === -1) {
      return `
        <div class='link-to-member-avatar'>
          <a class="author_link has-tooltip approver-avatar js-approver-list-member" data-container="body" href="/${user.username}" data-original-title="Approved by ${user.name}">
            <img width="30" class="avatar avatar-inline s20 js-lazy-loaded w30 red" alt="" src="${user.avatar_url}">
            <div class="avatar-award-emoji">${emoji ? emoji.name : ''}</div>
          </a>
        </div>`
    }
