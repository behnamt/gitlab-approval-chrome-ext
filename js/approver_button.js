...
function getCurrentApprovals() {
    ...
    return getApprovals(projectId, mergeRequestId)
        .then(function (mergeRequest) {
            ...
            approvals_required = mergeRequest.approvals_required
        })
}
...
function addCurrentUserAsApprover(apiKey) {
  ...
  findDefaultApprovalRule().then(function (default_rule) {
    if (default_rule === undefined) {
        createDefaultApprovalRule(apiKey, projectId, mergeRequestId, current_approvers, approvals_required);
    } else {
        updateDefaultApprovalRule(apiKey, projectId, mergeRequestId, default_rule.id, current_approvers, approvals_required);
    }
  });
...
async function findDefaultApprovalRule() {
  let projectId = getProjectId();
  let mergeRequestId = window.location.pathname.split('/')[4];
  let rule = {};
  await getApprovalRules(projectId, mergeRequestId)
    .then(function (rules) {
        rule = rules.find(rule => rule.name === "Default");
    });
  return rule;
}
